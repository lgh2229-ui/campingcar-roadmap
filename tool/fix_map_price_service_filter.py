...
new = """  bool _isUnknownPrice(String value) {\n    final v = value.trim();\n    return v.isEmpty || v.contains('확인 필요') || v.contains('정보 없음') || v.contains('금액정보 없음');\n  }\n\n  bool _serviceMatchesPrice(Place p, String service) {\n    final value = '${p.prices[service] ?? ''}'.trim();\n    if (priceFilter == '전체') return true;\n    if (_isUnknownPrice(value)) return false;\n    final isFree = value.contains('무료');\n    return priceFilter == '무료' ? isFree : !isFree;\n  }\n\n  List<Place> get visiblePlaces {\n    return places.where((p) {\n      if (filter != '전체') {\n        if (!p.services.contains(filter)) return false;\n        return _serviceMatchesPrice(p, filter);\n      }\n      if (priceFilter == '전체') return true;\n      for (final service in p.services) {\n        if (_serviceMatchesPrice(p, service)) return true;\n      }\n      return false;\n    }).toList();\n  }\n\n  Future<void> _openMapFilterMenu(String price) async {\n    final selected = await showModalBottomSheet<String>(\n      context: context,\n      showDragHandle: true,\n      builder: (ctx) => SafeArea(\n        child: Column(\n          mainAxisSize: MainAxisSize.min,\n          children: [\n            Padding(\n              padding: const EdgeInsets.fromLTRB(18, 4, 18, 10),\n              child: Row(children: [\n                Expanded(child: Text('$price 장소 필터', style: const TextStyle(fontSize: 19, fontWeight: FontWeight.bold))),\n                Text('서비스 선택', style: TextStyle(color: Theme.of(ctx).colorScheme.onSurfaceVariant)),\n              ]),\n            ),\n            ...serviceFilters.map((service) => ListTile(\n                  leading: Icon(service == '전체' ? Icons.apps : service == '블랙탱크 비움' ? Icons.delete_outline : service == '급수' ? Icons.water_drop_outlined : service == '노지/차박' ? Icons.local_parking : Icons.wc),\n                  title: Text(_filterLabel(service)),\n                  trailing: priceFilter == price && filter == service ? const Icon(Icons.check) : null,\n                  onTap: () => Navigator.pop(ctx, service),\n                )),\n            const SizedBox(height: 8),\n          ],\n        ),\n      ),\n    );\n    if (selected == null || !mounted) return;\n    setState(() {\n      priceFilter = price;\n      filter = selected;\n    });\n  }\n\n  Widget _mapFilterButton(String price) {\n    final active = priceFilter == price;\n    final sub = active && filter != '전체' ? ' · ${_filterLabel(filter)}' : '';\n    return Padding(\n      padding: const EdgeInsets.only(right: 6),\n      child: FilterChip(\n        label: Row(mainAxisSize: MainAxisSize.min, children: [Text('$price$sub'), const SizedBox(width: 2), const Icon(Icons.arrow_drop_down, size: 18)]),\n        selected: active,\n        onSelected: (_) => _openMapFilterMenu(price),\n      ),\n    );\n  }\n"""
if old not in s:
...
new_ui = """          SingleChildScrollView(\n            scrollDirection: Axis.horizontal,\n            child: Row(children: ['전체', '유료', '무료'].map(_mapFilterButton).toList()),\n          ),"""
if old_ui not in s:
    raise SystemExit('map filter UI block not found')
s = s.replace(old_ui, new_ui, 1)

p.write_text(s, encoding='utf-8')
print('patched map paid/free filters with service submenus')
