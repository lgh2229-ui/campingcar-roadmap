...
logic=r'''  bool _isMine(Place p) => p.ownerId == _currentAuthorId;
  ...
  Future<void> _showMapFilterDialog() async { final draft=Set<String>.from(selectedMapFilters); await showDialog<void>(context:context,builder:(ctx)=>StatefulBuilder(builder:(ctx,setLocal)=>AlertDialog(title:const Text('지도 필터'),content:SizedBox(width:420,child:SingleChildScrollView(child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisSize:MainAxisSize.min,children:[CheckboxListTile(contentPadding:EdgeInsets.zero,value:draft.isEmpty,title:const Text('전체',style:TextStyle(fontWeight:FontWeight.bold)),onChanged:(_)=>setLocal(()=>draft.clear())),const Divider(),const Text('캠핑·편의',style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)),Wrap(spacing:6,children:campingFilters.map((e)=>FilterChip(label:Text(_filterLabel(e)),selected:draft.contains(e),onSelected:(v)=>setLocal((){if(v)draft.add(e);else draft.remove(e);}))).toList()),const SizedBox(height:18),const Text('캠핑카 서비스',style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)),Wrap(spacing:6,children:businessFilters.map((e)=>FilterChip(label:Text(e),selected:draft.contains(e),onSelected:(v)=>setLocal((){if(v)draft.add(e);else draft.remove(e);}))).toList())]))),actions:[TextButton(onPressed:()=>setLocal(()=>draft.clear()),child:const Text('초기화')),FilledButton(onPressed:(){setState((){selectedMapFilters..clear()..addAll(draft);});Navigator.pop(ctx);},child:const Text('적용'))])))); }
  Widget _serviceAvailability(Place p){ const items=<(String,String)>[('블랙탱크 비움','블랙탱크'),('급수','급수'),('노지/차박','노지/차박'),('공중화장실','화장실'),('제작','제작'),('매매','매매'),('A/S','A/S'),('부품·용품','부품/용품')]; return Card(margin:const EdgeInsets.only(top:10),child:Padding(padding:const EdgeInsets.all(12),child:Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const Text('캠핑카 서비스',style:TextStyle(fontWeight:FontWeight.bold)),const SizedBox(height:8),Wrap(spacing:7,runSpacing:7,children:items.map((x){final yes=p.services.contains(x.$1);return Chip(avatar:Icon(yes?Icons.check_circle:Icons.remove_circle_outline,size:18),label:Text(x.$2));}).toList())]))); }
'''
...
# Full requested place-detail ordering. Existing service-price widget becomes 이용안내 below 캠핑카 서비스.
...
# Add visual separation between contact and usage instructions.
