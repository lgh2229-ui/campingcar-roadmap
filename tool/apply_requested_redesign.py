      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx, setLocal) => AlertDialog(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: campingFilters.map((e) => FilterChip(
                      label: Text(_filterLabel(e)),
                      selected: draft.contains(e),
                  ...
                  Wrap(
                    spacing: 6,
                    runSpacing: 6,
                    children: businessFilters.map((e) => FilterChip(
                      label: Text(e),
                      selected: draft.contains(e),
                      onSelected: (v) => setLocal(() { if (v) { draft.add(e); } else { draft.remove(e); } }),
                    )).toList(),
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () {
                setState(() { selectedMapFilters..clear()..addAll(draft); });
                Navigator.pop(ctx);
              },
              child: const Text('적용'),
            ),
  ...
  Widget _serviceAvailability(Place p) {
    ...
    return Column(
      ...
      children: [
        ...
        ...items.map((x) {
          final available = p.services.contains(x.$1);
          return Padding(
            padding: const EdgeInsets.symmetric(vertical: 2),
            child: Row(children: [
              SizedBox(width: 105, child: Text('${x.$2} :', style: const TextStyle(fontWeight: FontWeight.w600))),
              Expanded(child: Text(available ? '가능' : '이용불가')),
            ]),
          );
        }),
      ],
...
# Insert camper service section immediately before the existing usage section.
...
print('requested redesign applied cleanly')
