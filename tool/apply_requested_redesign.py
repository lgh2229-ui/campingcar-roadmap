...
logic=r'''  bool _isMine(Place p) => p.ownerId == _currentAuthorId;
  List<Place> get visiblePlaces { final approved=places.where((p)=>p.isApproved).toList(); if(selectedMapFilters.isEmpty)return approved; return approved.where((p)=>p.services.any(selectedMapFilters.contains)).toList(); }
  Future<void> _showMapFilterDialog() async { final draft=Set<String>.from(selectedMapFilters); await showDialog<void>(context:context,builder:(ctx)=>StatefulBuilder(builder:(ctx,setLocal)=>AlertDialog(title:const Text('지도 필터'),content:SizedBox(width:420,child:SingleChildScrollView(child:Column(crossAxisAlignment:CrossAxisAlignment.start,mainAxisSize:MainAxisSize.min,children:[CheckboxListTile(contentPadding:EdgeInsets.zero,value:draft.isEmpty,title:const Text('전체',style:TextStyle(fontWeight:FontWeight.bold)),onChanged:(_)=>setLocal(()=>draft.clear())),const Divider(),const Text('캠핑·편의',style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)),Wrap(spacing:6,runSpacing:6,children:campingFilters.map((e)=>FilterChip(label:Text(_filterLabel(e)),selected:draft.contains(e),onSelected:(v)=>setLocal((){if(v){draft.add(e);}else{draft.remove(e);}}))).toList()),const SizedBox(height:18),const Text('캠핑카 서비스',style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)),Wrap(spacing:6,runSpacing:6,children:businessFilters.map((e)=>FilterChip(label:Text(e),selected:draft.contains(e),onSelected:(v)=>setLocal((){if(v){draft.add(e);}else{draft.remove(e);}}))).toList())]))),actions:[TextButton(onPressed:()=>setLocal(()=>draft.clear()),child:const Text('초기화')),FilledButton(onPressed:(){setState((){selectedMapFilters..clear()..addAll(draft);});Navigator.pop(ctx);},child:const Text('적용'))])))); }
  Widget _serviceAvailability(Place p){ const items=<(String,String)>[('제작','제작'),('매매','매매'),('A/S','A/S'),('부품·용품','부품/용품')]; return Column(crossAxisAlignment:CrossAxisAlignment.start,children:[const SizedBox(height:8),const Text('캠핑카 서비스',style:TextStyle(fontSize:16,fontWeight:FontWeight.bold)),const SizedBox(height:6),...items.map((x){final available=p.services.contains(x.$1);return Padding(padding:const EdgeInsets.symmetric(vertical:2),child:Row(children:[SizedBox(width:105,child:Text('${x.$2} :',style:const TextStyle(fontWeight:FontWeight.w600))),Expanded(child:Text(available?'가능':'이용불가'))]));})]); }
'''
...
if is_>=0 and ie>=0:s=s[:is_]+'''  List<String> _icons(Place p) {\n    final out=<String>[];\n    if(p.services.contains('노지/차박'))out.add('🏕️'); if(p.services.contains('공중화장실'))out.add('🚻'); if(p.services.contains('급수'))out.add('💧'); if(p.services.contains('블랙탱크 비움'))out.add('🚽'); if(p.services.contains('제작'))out.add('🏭'); if(p.services.contains('매매'))out.add('🚐'); if(p.services.contains('A/S'))out.add('🔧'); if(p.services.contains('부품·용품'))out.add('🧰'); return out.isEmpty?['📍']:out;\n  }\n\n'''+s[ie:]
...
# Force service section directly before the existing 이용안내 block generated by show_service_prices.py.
if '_serviceAvailability(p),' not in s:
 anchors=["        const Text('이용가능 서비스', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),","        const Text('이용안내', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),"]
 inserted=False
 for a in anchors:
  if a in s:s=s.replace(a,"        _serviceAvailability(p),\n"+a,1);inserted=True;break
 if not inserted:raise SystemExit('cannot locate usage information block for camper service insertion')
...
p.write_text(s,encoding='utf-8');print('forced camper service section before usage information')
