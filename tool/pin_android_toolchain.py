...
if wrapper.exists():
    ...
    s, n = re.subn(r'distributionUrl=.*gradle-[^-/]+-all\.zip',
                   'distributionUrl=https\\://services.gradle.org/distributions/gradle-8.14-all.zip', s, count=1)
    if n != 1:
        s, n = re.subn(r'distributionUrl=.*gradle-[^-/]+-bin\.zip',
                       'distributionUrl=https\\://services.gradle.org/distributions/gradle-8.14-bin.zip', s, count=1)
    if n != 1:
...
print('Pinned Android build stack: AGP 8.13.1 / Gradle 8.14')
