...
if 'Future<AppUser> updateProfile({required String nickname' not in s:
    marker = '  Future<AppUser> updateVehicle('
    pos = s.find(marker)
    if pos < 0:
        raise RuntimeError('updateVehicle marker not found in AuthRepository')
    ...
    if (password.isNotEmpty && !validPassword(password)) throw Exception('새 비밀번호는 영문, 숫자, 특수문자를 포함한 8~20자로 입력해주세요.');
    if (!serverEnabled) {
    ...
    if (password.isNotEmpty) await client!.auth.updateUser(UserAttributes(password: password));
...
method = r'''  Future<void> _passwordGate() async {
    ...
    final entered = await showDialog<String>(context: context, builder: (ctx) => AlertDialog(
      title: const Text('비밀번호 확인'),
      content: TextField(controller: c, obscureText: true, autofocus: true, decoration: const InputDecoration(labelText: '현재 비밀번호'), onSubmitted: (_) => Navigator.pop(ctx, c.text)),
      actions: [TextButton(onPressed: () => Navigator.pop(ctx), child: const Text('취소')), FilledButton(onPressed: () => Navigator.pop(ctx, c.text), child: const Text('확인'))],
    ));
  ...
  Future<void> _profileEditDialog() async {
    ...
    await showDialog<void>(context: context, barrierDismissible: false, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: savingProfile ? null : () async {
          final nick = nickname.text.trim();
          if (nick.isEmpty) return _msg('닉네임을 입력해주세요.');
          if (newPassword.text.isNotEmpty && newPassword.text != confirmPassword.text) return _msg('새 비밀번호 확인이 일치하지 않습니다.');
        ...
        }, child: savingProfile ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) : const Text('저장')),
      ],
    )));
  }
