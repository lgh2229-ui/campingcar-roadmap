...
if 'Future<List<Map<String,dynamic>>> _notificationRows()' not in s:
  ...
  Future<void> _openNotifications() async {
    ...
    await showModalBottomSheet<void>(context:context,showDragHandle:true,isScrollControlled:true,builder:(sheetContext)=>StatefulBuilder(builder:(sheetContext,setSheetState)=>SafeArea(child:SizedBox(height:MediaQuery.of(sheetContext).size.height*.78,child:Column(children:[
      ...
      Expanded(child:rows.isEmpty?const Center(child:Text('새 알림이 없습니다.')):ListView.separated(itemCount:rows.length,separatorBuilder:(_,__)=>const Divider(height:1),itemBuilder:(_,i){final row=rows[i],unread=row['is_read']!=true;return ListTile(leading:Icon(unread?Icons.notifications_active:Icons.notifications_none),title:Text(row['title']?.toString()??'',style:TextStyle(fontWeight:unread?FontWeight.bold:FontWeight.normal)),subtitle:Text([row['body']?.toString()??'',row['created_at']?.toString()??''].join(' · ')),isThreeLine:true,onTap:()async{if(unread){await db.from('notifications').update({'is_read':true}).eq('id',row['id']);setSheetState(()=>rows[i]=<String,dynamic>{...row,'is_read':true});if(mounted)setState((){});}});}))
    ]))))));
  }
...
notification_anchor="        Positioned(right: 14, bottom: 18, child: Column(children: ["
