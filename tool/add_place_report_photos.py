...
# Runs after the other UI patches so reporting cannot be overwritten.
...
old="db.from('vehicle_market_reports').select().order('created_at',ascending:false)])"
new="db.from('vehicle_market_reports').select().order('created_at',ascending:false),db.from('place_reports').select().order('created_at',ascending:false)])"
