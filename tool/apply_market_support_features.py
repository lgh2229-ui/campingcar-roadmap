...
# Only apply the small home-screen integrations here.  The checked-in vehicle
# market screen is the source of truth and must never be rewritten during CI.
...
if needle in s:
    ...
    for a in ["        if (p.note.isNotEmpty) Text('이용방법/주의사항: ${p.note}'),","        if (p.note.isNotEmpty) Text('이용방법 / 주의사항: ${p.note}'),"]:
        if a in s:
            s=s.replace(a,a+"\n        const SizedBox(height: 16),\n        _serviceAvailability(p),",1)
            break
p.write_text(s,encoding='utf-8')
print('market support integrations applied; checked-in vehicle market source preserved')
