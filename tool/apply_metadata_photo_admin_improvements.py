...
# Final CI compatibility pass. Source files now contain most requested behavior;
# this only repairs legacy generated source without undoing direct changes.
...
# If an older patch used parent setState, force the dialog-local state callback.
s=s.replace('onTap:(){photos.removeAt(i);setState((){});}','onTap:()=>setS(()=>photos.removeAt(i))')
# metadata in place detail
...
# admin delete button (actual deletion; edit button is kept only when an edit implementation exists)
...
# Vehicle market: ensure photo append/delete and edit cancel survive legacy source.
...
p.write_text(s,encoding='utf-8')
print('final requested UI compatibility fixes applied')
