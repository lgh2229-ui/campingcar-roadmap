...
# This is the FINAL feature patch before Flutter setup. It intentionally does not
# rewrite the working place-photo picker; it only verifies it survives.
p=Path('lib/screens/home_screen.dart')
s=p.read_text(encoding='utf-8')

# Add an admin-only place editor. Keep photos completely untouched.
...
# Metadata shown on every place detail.
# requested redesign has already run, so accept either address rendering form.
meta="        Text('등록자: ${p.ownerNickname.trim().isEmpty ? '닉네임 없음' : p.ownerNickname}'),\n        Text('최종 등록일자: ${p.createdAt==null ? '-' : p.createdAt!.toLocal().toString().substring(0,16)}'),"
if "Text('최종 등록일자:" not in s:
    for detail in ["        if (p.address.isNotEmpty) Text('주소 : ${p.address}'),","        if (p.address.isNotEmpty) Text(p.address),"]:
        if detail in s:
            s=s.replace(detail,detail+'\n'+meta,1);break
    else: raise SystemExit('place detail address anchor missing')

# Admin-only edit/delete buttons. They are not built at all for ordinary users.
...
# Verify the known-good photo feature has NOT been damaged.
...
# Verify requested place-detail/admin features in final HomeScreen.
...
# Verify management screen already carries requested market/member/feedback/report fields.
m=Path('lib/screens/admin_management_screen.dart').read_text(encoding='utf-8')
management_required=[
 "아이디 · 닉네임 통합검색",
 "'${_userId(x)} ${_nickname(x)}'.toLowerCase().contains(q)",
 "마지막접속일자 및 시간",
 "x['last_seen_at']",
 "x['status']=='sold'?'판매완료':'판매중'",
 "등록자 ${_name(x['owner_id'])}",
 "${_dt(x['created_at'])}",
 "작성자: ${_name(x['author_id'])} · 요청:",
 "신고자: ${_name(x['reporter_id'])} · 요청:",
]
...
# Verify last-seen is actually written on successful server login.
...
print('OK: requests 3-9 verified; working multi-photo feature preserved')
