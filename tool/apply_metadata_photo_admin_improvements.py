...
# PLACE REGISTRATION: append photos and rebuild the dialog immediately on X delete.
...
# Place detail metadata.
detail="        if (p.address.isNotEmpty) Text(p.address),"
meta="""        if (p.address.isNotEmpty) Text(p.address),
        Text('등록자: ${p.ownerNickname.trim().isEmpty ? '닉네임 없음' : p.ownerNickname}'),
        if(p.createdAt!=null) Text('최종 등록일자: ${p.createdAt!.toLocal().toString().substring(0,16)}'),"""
if detail in s and "최종 등록일자:" not in s:s=s.replace(detail,meta,1)
# Admin-only controls on place detail. Update uses existing admin editor elsewhere; delete is repository-backed.
needle="        if (!p.isApproved && _isMine(p)) ...["
admin="""        if(widget.user.isAdministrator) ...[
          const SizedBox(height:10),
          Row(children:[Expanded(child:OutlinedButton.icon(onPressed:(){Navigator.pop(ctx);setState(()=>selectedSpot=LatLng(p.latitude,p.longitude));_msg('관리자 수정은 관리자 승인화면의 수정 버튼을 이용해주세요.');},icon:const Icon(Icons.edit),label:const Text('수정'))),const SizedBox(width:8),Expanded(child:FilledButton.tonalIcon(onPressed:()async{final ok=await showDialog<bool>(context:context,builder:(d)=>AlertDialog(title:const Text('장소 삭제'),content:Text('${p.name} 장소를 삭제할까요?'),actions:[TextButton(onPressed:()=>Navigator.pop(d,false),child:const Text('취소')),FilledButton(onPressed:()=>Navigator.pop(d,true),child:const Text('삭제'))]));if(ok==true){await widget.data.deletePlace(p.id);if(ctx.mounted)Navigator.pop(ctx);await _load();_msg('장소를 삭제했습니다.');}},icon:const Icon(Icons.delete_outline),label:const Text('삭제')))]),
        ],
"""
if needle in s and '장소를 삭제했습니다.' not in s:s=s.replace(needle,admin+needle,1)
p.write_text(s,encoding='utf-8')

# MARKET: append photos, removable new thumbnails, edit cancel, owner nickname/date/status in cards/detail.
...
# Add metadata to common subtitle status occurrence.
s=s.replace("subtitle:Text(x['status']=='sold'?'판매완료':'판매중')", "subtitle:Text('${x['status']=='sold'?'판매완료':'판매중'} · 등록자 ${('${x['owner_nickname']??''}').isEmpty?'닉네임 없음':x['owner_nickname']} · 등록 ${('${x['created_at']??''}').length>=16?'${x['created_at']}'.substring(0,16):'${x['created_at']??''}'}')")
p.write_text(s,encoding='utf-8')

# ADMIN MANAGEMENT: ID/nickname only search, last seen, requester nickname/time.
...
# ADMIN APPROVAL: place registrant nickname + request time.
p=Path('lib/screens/admin_screen.dart'); s=p.read_text(encoding='utf-8')
needle="                if (p.address.isNotEmpty) Text(p.address),"
rep="""                if (p.address.isNotEmpty) Text(p.address),
                Text('등록자: ${p.ownerNickname.trim().isEmpty ? '닉네임 없음' : p.ownerNickname}'),
                if(p.createdAt!=null) Text('요청일시: ${p.createdAt!.toLocal().toString().substring(0,16)}'),"""
if needle in s and "요청일시:" not in s:s=s.replace(needle,rep,1)
p.write_text(s,encoding='utf-8')
print('all requested metadata/photo/admin UI fixes applied')
