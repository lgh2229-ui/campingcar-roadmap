...
# PLACE REGISTRATION + DETAIL. The photo picker is inside a StatefulBuilder, but
# some older patches can move the thumbnail block outside its setS lexical scope.
# Use the parent State.setState for X removal; it is always valid and updates the UI.
...
# Repair any thumbnail generated by an earlier patch in the same workflow.
s=s.replace('onTap:()=>setS(()=>photos.removeAt(i))','onTap:(){photos.removeAt(i);setState((){});}')
...
p.write_text(s,encoding='utf-8')
print('requested fixes applied; photo delete callback analyzer-safe')
