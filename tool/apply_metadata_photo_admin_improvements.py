...
add=s[start:end]
required=['builder: (pageContext, setPageState)','pickMultiImage(imageQuality: 82)','setPageState(() { photos.addAll(additions); })','ValueKey(photo.path)','Image.file(File(photo.path)','setPageState(() { photos.removeAt(i); })']
missing=[x for x in required if x not in add]
if 'photos.clear()' in add or 'setS(() { photos.' in add or missing:
    raise SystemExit('VALIDATION FAILED: registration photo local-state UI: '+','.join(missing))
...
p.write_text(s,encoding='utf-8')
print('validated registration-local multi-photo add/preview/delete')
