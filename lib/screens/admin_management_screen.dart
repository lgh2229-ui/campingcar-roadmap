...
class _S extends State<AdminManagementScreen> with SingleTickerProviderStateMixin {
  ...
  String _memberTitle(Map<String,dynamic> x) { final id=_userId(x), nick=_nickname(x); if(id.isNotEmpty&&nick.isNotEmpty)return '$id · $nick'; if(id.isNotEmpty)return id; if(nick.isNotEmpty)return nick; return '회원정보 없음'; }
  List<Map<String,dynamic>> get _filteredMembers { final q=memberQuery.trim().toLowerCase(); if(q.isEmpty)return members; return members.where((x)=>[_userId(x),_nickname(x),'${x['phone']??''}','${x['vehicle_name']??''}','${x['role']??''}','${x['account_status']??''}'].join(' ').toLowerCase().contains(q)).toList(); }
  ...
  Future<void> _showMember(Map<String,dynamic> x) async {
    final fields=<MapEntry<String,String>>[MapEntry('아이디',_userId(x)),MapEntry('닉네임',_nickname(x)),MapEntry('휴대폰','${x['phone']??''}'),MapEntry('휴대폰 인증',x['phone_verified']==true?'완료':'미인증'),MapEntry('회원상태','${x['account_status']??'active'}'),MapEntry('권한','${x['role']??'user'}'),MapEntry('차량상태','${x['vehicle_status']??''}'),MapEntry('차량명/모델','${x['vehicle_name']??''}'),MapEntry('차량높이',x['vehicle_height_mm']==null?'':'${(x['vehicle_height_mm'] as num)/1000} m'),MapEntry('위생설비','${x['sanitation_type']??''}'),MapEntry('가입일','${x['created_at']??''}')];
    await showModalBottomSheet<void>(context:context,showDragHandle:true,isScrollControlled:true,builder:(ctx){return SafeArea(child:SizedBox(height:MediaQuery.of(ctx).size.height*.78,child:Column(crossAxisAlignment:CrossAxisAlignment.stretch,children:[Padding(padding:const EdgeInsets.fromLTRB(20,4,20,12),child:Text(_memberTitle(x),style:const TextStyle(fontSize:20,fontWeight:FontWeight.bold))),const Divider(height:1),Expanded(child:ListView(children:fields.where((e)=>e.value.trim().isNotEmpty).map<Widget>((e)=>ListTile(title:Text(e.key),subtitle:Text(e.value))).toList()))])));});
  }

  Widget _memberTab(){final rows=_filteredMembers;return Column(children:[Padding(padding:const EdgeInsets.all(12),child:TextField(controller:memberSearch,onChanged:(v)=>setState(()=>memberQuery=v),decoration:InputDecoration(prefixIcon:const Icon(Icons.search),hintText:'아이디 · 닉네임 · 휴대폰 · 차량 통합검색',suffixIcon:memberQuery.isEmpty?null:IconButton(onPressed:(){memberSearch.clear();setState(()=>memberQuery='');},icon:const Icon(Icons.close)),border:const OutlineInputBorder()))),Expanded(child:rows.isEmpty?const Center(child:Text('검색 결과가 없습니다.')):ListView.builder(itemCount:rows.length,itemBuilder:(_,i){final x=rows[i];return ListTile(leading:const CircleAvatar(child:Icon(Icons.person)),title:Text(_memberTitle(x)),subtitle:Text('상태: ${x['account_status']??'active'}'),onTap:()=>_showMember(x),trailing:PopupMenuButton<String>(onSelected:(v)=>_member(x,v),itemBuilder:(_)=>const[PopupMenuItem(value:'active',child:Text('정상 복구')),PopupMenuItem(value:'suspended',child:Text('자격정지')),PopupMenuItem(value:'withdrawn',child:Text('탈퇴 처리'))]));}))]);}

  @override Widget build(BuildContext context)=>Scaffold(appBar:AppBar(title:const Text('관리자 통합관리'),bottom:TabBar(controller:tabs,isScrollable:true,tabs:const[Tab(text:'중고매물'),Tab(text:'회원'),Tab(text:'의견'),Tab(text:'신고')])),body:loading?const Center(child:CircularProgressIndicator()):TabBarView(controller:tabs,children:[ListView.builder(itemCount:market.length,itemBuilder:(_,i){final x=market[i];return ListTile(title:Text('${x['title']}'),subtitle:Text(x['status']=='sold'?'판매완료':'판매중'),trailing:PopupMenuButton<String>(onSelected:(v){if(v=='edit')_editMarket(x);if(v=='delete')_deleteMarket(x);},itemBuilder:(_)=>const[PopupMenuItem(value:'edit',child:Text('수정')),PopupMenuItem(value:'delete',child:Text('삭제'))]));}),_memberTab(),ListView.builder(itemCount:feedback.length,itemBuilder:(_,i){final x=feedback[i];return ListTile(title:Text('${x['title']}'),subtitle:Text('${x['body']}\n${x['status']=='answered'?'답변완료':'답변대기'}'),isThreeLine:true,onTap:()=>_reply(x));}),ListView.builder(itemCount:reports.length,itemBuilder:(_,i){final x=reports[i];return ListTile(leading:const Icon(Icons.report),title:Text('매물 ${x['listing_id']}'),subtitle:Text('${x['reason']}'),trailing:Text('${x['status']}'));})]));
}
