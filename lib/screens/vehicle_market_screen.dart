...
class _VehicleMarketScreenState extends State<VehicleMarketScreen> {
  ...
  Future<void> filters() async {
    ...
    final a=c(minPrice),b=c(maxPrice),y1=c(minYear),y2=c(maxYear),km=c(maxKm),ba=c(minBattery),so=c(minSolar),inv=c(minInverter),alt=c(minAlternator),fr=c(minFresh),gr=c(minGrey),tc=c(minToilet),rg=TextEditingController(text:region); var st=status,tt=toilet; bool? sh=shore;
