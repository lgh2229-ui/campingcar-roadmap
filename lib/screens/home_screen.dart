...
class _HomeScreenState extends State<HomeScreen> {
  ...
  Widget _mapPage() => Stack(children: [
        FlutterMap(
          ...
          children: [
            ...
            MarkerLayer(markers: visiblePlaceGroups.map((group) {
              final p = group.first;
        ...
        Positioned(top: 10, left: 10, right: 10, child: Column(children: [
          ...
          SingleChildScrollView(scrollDirection: Axis.horizontal, child: Row(children: ['전체', '블랙탱크 비움', '급수', '노지/차박', '공중화장실'].map((e) => Padding(
            padding: const EdgeInsets.only(right: 6),
            child: ChoiceChip(label: Text(e == '블랙탱크 비움' ? '블랙' : e == '공중화장실' ? '화장실' : e), selected: filter == e, onSelected: (_) => setState(() => filter = e)),
          )).toList())),
        ])),
        Positioned(right: 14, bottom: 18, child: Column(children: [
          ...
          FloatingActionButton.extended(heroTag: 'add', onPressed: _openAddPlace, icon: const Icon(Icons.add_location_alt_outlined), label: const Text('장소등록')),
        ])),
      ]);

  Widget _savedPage() {
    final rows = places.where((p) => saved.contains(p.id)).toList();
    return Scaffold(
      appBar: AppBar(title: const Text('저장한 장소')),
      body: rows.isEmpty ? const Center(child: Text('저장한 장소가 없습니다.')) : ListView.builder(
  ...
  Future<void> _openAddPlace() async {
    ...
    final address = TextEditingController(text: await _reverseAddress(center));
    ...
    await showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
      ...
      content: SizedBox(width: 440, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        DropdownButtonFormField<String>(value: reservation, items: ['예약불필요', '예약필수', '전화문의'].map((e) => DropdownMenuItem(value: e, child: Text(e))).toList(), onChanged: (v) => setS(() => reservation = v!), decoration: const InputDecoration(labelText: '예약 여부')),
    ...
    await showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
      ...
      content: SizedBox(width: 440, child: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        DropdownButtonFormField<String>(
          value: ['예약불필요', '예약필수', '전화문의'].contains(reservation) ? reservation : '예약불필요',
