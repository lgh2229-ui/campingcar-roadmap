...
class SupabaseRepository implements AppDataRepository {
  ...
  @override Future<List<Place>> pendingPlaces() async { final rows=await client.from('places_view').select().eq('approval_status','pending').order('created_at',ascending:false); return (rows as List).map((e)=>Place.fromJson(Map<String,dynamic>.from(e))).toList(); }
  ...
  @override Future<void> updateReview({required String reviewId,required String status,required String body}) async { final updated=await client.from('reviews').update({'status':status,'body':body.trim()}).eq('id',reviewId).eq('author_id',_uid).select('id'); if((updated as List).isEmpty) throw Exception('본인이 작성한 리뷰만 수정할 수 있습니다.'); }
  @override Future<void> deleteReview(String reviewId) async { final deleted=await client.from('reviews').delete().eq('id',reviewId).eq('author_id',_uid).select('id'); if((deleted as List).isEmpty) throw Exception('본인이 작성한 리뷰만 삭제할 수 있습니다.'); }
  ...
  @override
  Future<List<AdminReviewTask>> adminReviewTasks({bool includeHandled = false}) async {
    var query = client.from('review_admin_tasks').select();
    if (!includeHandled) query = query.eq('handled', false);
