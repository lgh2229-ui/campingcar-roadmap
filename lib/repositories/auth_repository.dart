...
class AuthRepository {
  ...
  Future<String> sendMockSms(String phone) async {
    final normalized = normalizePhone(phone);
    final code = (100000 + Random().nextInt(900000)).toString();
    _smsCodes[normalized] = code;
    return code;
  }
  ...
  Future<AppUser?> login(String userId, String password) async {
    if (!serverEnabled) {
      ...
      for (final u in users) {
        if (u.userId.toLowerCase() == userId.toLowerCase() && u.password == password) {
          await local.setSession(u.userId);
          return u;
        }
      }
      return null;
    ...
    try {
      await client!.auth.signInWithPassword(email: loginEmail(userId), password: password);
  ...
  Future<void> signup(AppUser user) async {
    if (serverEnabled) {
      throw UnsupportedError('서버 모드에서는 beginServerSignup/verifyServerSignupPhone을 사용합니다.');
    }
    final users = await local.users();
    if (users.any((u) => u.userId.toLowerCase() == user.userId.toLowerCase())) {
  ...
  Future<void> beginServerSignup({
    ...
    if (id.toLowerCase() == 'administrator') {
      throw Exception('administrator 아이디는 관리자 전용이라 일반 회원가입으로 만들 수 없습니다.');
    }
    if (!RegExp(r'^[A-Za-z0-9_]{4,20}$').hasMatch(id)) {
    ...
    if (authUser == null) throw Exception('회원 계정을 만들지 못했습니다.');
  ...
  Future<AppUser?> currentUser() async {
    if (!serverEnabled) {
      final id = await local.sessionUserId();
    ...
    final row = await client!.from('profiles').select().eq('id', authUser.id).maybeSingle();
    if (row == null) return null;
    return AppUser.fromJson(Map<String, dynamic>.from(row));
  }

  Future<void> sendRecoveryOtp(String phone) async {
    if (!serverEnabled) return;
    await client!.auth.signInWithOtp(phone: toE164(phone), shouldCreateUser: false);
  }

  Future<void> verifyRecoveryOtp(String phone, String token) async {
    if (!serverEnabled) return;
    await client!.auth.verifyOTP(type: OtpType.sms, phone: toE164(phone), token: token.trim());
  }
  ...
  Future<String?> findIdByPhone(String phone) async {
    if (!serverEnabled) {
      ...
      final users = await local.users();
      for (final u in users) {
        if (normalizePhone(u.phone) == normalized) return u.userId;
      }
      return null;
    }
    final u = await currentUser();
    if (u == null || normalizePhone(u.phone) != normalizePhone(phone)) return null;
    return u.userId;
  }

  Future<bool> resetPassword(String userId, String phone, String newPassword) async {
    if (!serverEnabled) {
      final users = await local.users();
      for (final u in users) {
        if (u.userId.toLowerCase() == userId.toLowerCase() && normalizePhone(u.phone) == normalizePhone(phone)) {
    ...
    if (profile == null || profile.userId.toLowerCase() != userId.toLowerCase() || normalizePhone(profile.phone) != normalizePhone(phone)) return false;
  ...
  Future<bool> reauthenticatePassword(String password) async {
    ...
    try {
      await client!.auth.signInWithPassword(email: loginEmail(profile.userId), password: password);
      return true;
    } on AuthException {
      return false;
    }
  ...
  Future<AppUser> updateVehicle({required String status, required String name, required int? heightMm, required String sanitation}) async {
    ...
    profile.vehicleStatus = status;
    profile.vehicleName = name;
    profile.vehicleHeightMm = heightMm;
    profile.sanitationType = sanitation;
    if (!serverEnabled) {
      final users = await local.users();
  ...
  Future<void> deleteAccount() async {
    if (!serverEnabled) {
      ...
      final users = await local.users();
      users.removeWhere((u) => u.userId == profile.userId);
      await local.saveUsers(users);
      await local.setSession(null);
      return;
    }
    ...
    if (data is Map && data['ok'] != true) {
      throw Exception('${data['error'] ?? '회원탈퇴 처리에 실패했습니다.'}');
    }
    await client!.auth.signOut();
  }
}
